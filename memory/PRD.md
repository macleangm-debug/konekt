...
111. **Premium price UI + discount state** — product card price block redesigned. Title gets its own line (no more side-by-side cramping); price sits below the sub-category chip as **TZS 135,000** in 2xl brand-primary extrabold font. When `original_price` (or `compare_at_price`) exceeds the live `customer_price`/`base_price`, the card automatically shows: strikethrough original, "Save TZS X,XXX (XX%)" pill, and a red "-XX%" corner badge on the image. ProductDetail page shows the same treatment at 4xl in the hero price block ("Promo price" label, "You save TZS X today" subline). Works purely from data — flip `original_price` to activate a promo anywhere in the catalog.
