...
### Codebase Pack 30 - Growth Engine (Commercial Rules + Revenue Execution) ✅ (March 22, 2026)

**Admin Control Panels:**
1. **Commission Engine** (`/admin/commission-engine`) - Unified affiliate + sales commission management
   - Protect minimum company markup first
   - Generate commission only after payment approval
   - Track all commissions in one ledger
   - Prevent over-allocation beyond distribution layer

2. **Promotion Engine** (`/admin/promotion-engine`) - Safe, margin-aware campaign creation
   - Campaign form: Title, Scope Key, Scope Type, Discount %
   - Recommended: up to ~5% where distribution layer supports it

3. **Payout Engine** (`/admin/payout-engine`) - Control thresholds and cash flow
   - Minimum threshold: 50,000 TZS
   - Payout cycle: monthly
   - Only approved commissions eligible
   - Admin approval before fund release

**Staff Dashboard:**
- **Sales Commission Dashboard** (`/staff/commission-dashboard`)
  - Metrics: Assigned, Closed, Earned, Pending, Conversion
  - Referral Tools: Sales can have their own link/code (SALE123)
  - Explains commission from protected distributable layer only

**Backend APIs:**
- `/api/promotion-engine/preview` - Calculate safe promotion budget
- `/api/promotion-engine/campaigns` - CRUD campaigns
- `/api/payout-engine/summary` - Payout eligibility check
- `/api/commission-engine-unified/create` - Create commission records
- `/api/attribution-engine/capture` - Track lead source attribution

**Key Business Rules Implemented:**
1. **Affiliate Lead → Reduced Sales Commission**
2. **Self-Generated Lead → Full Sales Commission**
3. **Company 20% Protected Markup** - Never touched
4. **Only Distributable Layer Shared** - Affiliate/sales/promo splits

**Attribution Types:**
- `website` - Direct website visitor
- `affiliate` - Referred by affiliate link/code
- `sales` - Self-generated by sales team
- `hybrid` - Both affiliate and sales involved

**Testing:**
- All tests passed (iteration_72.json)
- Backend: 100% (18/18 tests)
- Frontend: 100% (all 4 pages working)
