...
export default function CustomerSummaryCard({ customer }) {
  ...
  const isIndividual = (customer.type || customer.customer_type || "").toLowerCase() === "individual";
  ...
  return (
    <div className="rounded-2xl border bg-slate-50 p-5 space-y-3" data-testid="customer-summary-card">
      ...
      {isIndividual ? (
        <div className="text-base font-medium text-slate-800">{customer.contact_name || customer.name || "-"}</div>
      ) : (
        <>
          <div className="text-base font-medium text-slate-800">{customer.company_name || customer.name || "-"}</div>
          {customer.contact_name && (
            <div className="text-sm text-slate-600">{customer.contact_name}</div>
          )}
        </>
      )}
