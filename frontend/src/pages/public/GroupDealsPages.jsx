...
import { Clock, Users, Check, ArrowLeft, ShoppingCart, Shield } from "lucide-react";
...
export default function GroupDealDetailPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4" data-testid="join-success">
        <div className="max-w-md w-full text-center">
          <div className="w-20 h-20 rounded-full bg-green-100 flex items-center justify-center mx-auto mb-4"><Check className="w-10 h-10 text-green-600" /></div>
          ...
          <div className="bg-white rounded-2xl border p-5 text-left space-y-2 mb-4">
            <div className="flex justify-between text-sm"><span className="text-slate-500">Amount</span><span className="font-bold">{fmt(deal.discounted_price)}</span></div>
            <div className="flex justify-between text-sm"><span className="text-slate-500">Progress</span><span className="font-bold">{deal.current_committed}/{deal.display_target}</span></div>
          </div>
          <Link to="/group-deals" className="text-sm text-[#20364D] font-semibold hover:underline">Browse More Deals</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50" data-testid="deal-detail-page">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white rounded-2xl border p-5 space-y-4">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center gap-2 text-xs text-slate-500"><Shield className="w-3 h-3 text-green-500" /> Full refund if deal doesn't complete</div>
              ...
              {/* Join CTA */}
              {deal.status === "active" ? (
                <Button onClick={() => setShowJoin(true)} className="w-full bg-[#D4A843] hover:bg-[#c49933] text-[#20364D] font-bold py-3 text-base" data-testid="join-deal-btn">
      ...
      {/* Join Modal */}
      {showJoin && (
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4" onClick={() => setShowJoin(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()} data-testid="join-modal">
